...
func Load(dir string) (*types.Config, error) {
	routes, err := decodeFile[types.RoutesFile](filepath.Join(dir, "routes.yml"))
	if err != nil {
		return nil, err
	}
	imports, err := decodeFile[types.ImportsFile](filepath.Join(dir, "imports.yml"))
	if err != nil {
		return nil, err
	}
	functions, err := decodeFile[types.FunctionsFile](filepath.Join(dir, "functions.yml"))
	if err != nil {
		return nil, err
	}
	typesFile, err := decodeFile[types.TypesFile](filepath.Join(dir, "classes.yml"))
	if err != nil {
		return nil, err
	}
	controlFlow, err := decodeFile[types.ControlFlowFile](filepath.Join(dir, "controls.yml"))
	if err != nil {
		return nil, err
	}

	conditionalRules, err := decodeFile[types.ConditionalsFile](filepath.Join(dir, "conditionals.yml"))
	if err != nil {
		return nil, err
	}
